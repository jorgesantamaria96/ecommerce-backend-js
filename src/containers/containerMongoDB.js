...
// MongoDB Connection
const { MONGO_URI, ADVANCED_OPTIONS } = require("../configuration/mongodb.js");
const main = () => {
...
// Util Functions
const { asPOJO, renameField, removeField } = require("../utils/index.js");
...
class ContainerMongoDB {
  ...
  async update(id, newElem) {
    try {
      ...
      if (n == 0 || nModified == 0) {
        throw new Error(
          "Error, no se pudo actualizar el elemento porque no se encuentra elemento con id: " +
            newElem._id
        );
      } else {
